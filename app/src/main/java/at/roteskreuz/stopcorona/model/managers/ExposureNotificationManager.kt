...
import at.roteskreuz.stopcorona.model.services.ExposureMatchingAlarmService
            ...
            .subscribe { state ->
                ...
                when (state) {
                    is ExposureNotificationPhase.FrameworkRunning -> {
                        // enqueue the AlarmManager to run the exposure matching algorithm when framework is running
                        // this ensures to check exposure matching only if user started the framework at least once

                        ExposureMatchingAlarmService.enqueueExposurePeriodicMatching(contextInteractor.applicationContext)
                    }
                }
